    ...
    internal static class ChatCommandService
    {
            ...
            foreach (var command in comandList)
            {
                    ...
                    command.Value.HasFlag(ChatCommandAccessibility.Client))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            MyAPIGateway.Utilities.ShowMissionScreen($"Error in {command.Value.Name}", "Input: ", messageText, ex.ToString(), null, null);
            ...
            foreach (var command in comandList)
            {
                ...
                if (command.Value.HasFlag(ChatCommandAccessibility.Server))
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            MyAPIGateway.Utilities.SendMissionScreen(steamId, $"Error in {command.Value.Name}", "Input: ", messageText, ex.ToString(), null, null);
                            VRage.Utils.MyLog.Default.WriteLine($"##Mod## Admin Helper mod Exception caught. Message: {ex}");
                            continue;
                        }
        ...
        public static bool HasRight(ulong steamId, ChatCommand command)
        {
            if (command.HasFlag(ChatCommandAccessibility.Experimental))
                return ModConfigurationConsts.ExperimentalCreatorList.Contains(steamId) && command.Security <= _userSecurity;
